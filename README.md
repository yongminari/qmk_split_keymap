...
기본 타이핑 레이어입니다. 엄지 라인은 [Num/Sym - Navi - Func] 구조로 직관적인 완벽 대칭을 이룹니다. 알파 키들의 레이어 모디파이어 기능을 완전히 빼고 일반 키로 환원하여 타이핑 반응 속도를 최적화했습니다.
...
왼손 영역은 괄호 및 기호가 효율적으로 동선에 배치되었으며, 오른손 영역은 직관적인 텐키 패드입니다.
...
- **Key Position Changes for Optimization (타이핑 딜레이 최소화):**
  ...
  - **기호 및 괄호 최적 배치**:
    - `()` 소괄호: 왼쪽 엄지 바깥쪽 및 가운데에 배치.
    - `[]` 대괄호: 왼쪽 판 가장 바깥쪽 열의 중간과 아래에 세로 배치.
    - `{}` 중괄호: 왼쪽 판 가장 안쪽 2개 열(inner extra)에 상하 배치.
