...
└───────┴───────┴───────┼───────┼───────┼───────┤                       ├───────┼───────┼───────┼───────┴───────┴───────┘
                        ...
                        └───────┴───────┴───────┘                       └───────┴───────┴───────┘
```
*(G=GUI, A=Alt, C=Ctrl, S=Shift, TAB=Tab Key, ENT(F)=Enter hold for Func Layer, TAB(N)=Tab hold for Num Layer, SPC(N)=Space hold for Navi, LSFT/RSFT=Pure Left/Right Shift, RALT=Right Alt (Han/Eng))*
...
```text
┌───────┬───────┬───────┬───────┬───────┬───────┬───────┐       ┌───────┬───────┬───────┬───────┬───────┬───────┬───────┐
│       │  ESC  │ WH_L  │ WH_D  │ WH_U  │ WH_R  │ TG(R) │       │ TG(R) │ HOME  │ PGUP  │ PGDN  │  END  │       │       │
├───────┼───────┼───────┼───────┼───────┼───────┼───────┤       ├───────┼───────┼───────┼───────┼───────┼───────┼───────┤
│ ACL0  │       │ MS_L  │ MS_D  │ MS_U  │ MS_R  │       │       │       │ LEFT  │ DOWN  │  UP   │ RGHT  │       │       │
├───────┼───────┼───────┼───────┼───────┼───────┼───────┘       └───────┼───────┼───────┼───────┼───────┼───────┼───────┤
│ ACL2  │       │ BTN1  │ BTN3  │ BTN2  │       │                       │       │ BSPC  │  DEL  │       │       │  APP  │
└───────┴───────┴───────┼───────┼───────┼───────┤                       ├───────┼───────┼───────┼───────┴───────┴───────┘
                        │       │       │       │                       │       │       │       │
                        └───────┴───────┴───────┘                       └───────┴───────┴───────┘
```
*(ACL0/2=마우스 가속 속도 0/2, WH_L/D/U/R=마우스 휠 좌/하/상/우, MS_L/D/U/R=마우스 커서 이동 좌/하/상/우, BTN1/3/2=마우스 좌클릭/휠클릭/우클릭, BSPC=Backspace, DEL=Delete, TG(R)=RAW Toggle, APP=Context Menu)*
...
## ⌨️ Keymap Features
- **Home Row Mods (GAC):** `S,D,F / J,K,L` 키에 GUI, Alt, Ctrl 적용. 윈도우 환경(한/영, 한자 전환 간섭 및 AltGr 이슈) 호환성을 위해 **좌우 홈로우 모두 왼쪽 모디파이어(Left GUI/Alt/Ctrl)를 사용**합니다 (오판정이 잦은 새끼손가락 `A` 및 `;`에서 모디를 완전히 배제하고, 안쪽으로 시프트).
- **Tapping Term:** 180ms 일괄 적용.
- **Tapping Optimizations:** 오작동 방지를 위해 `HOLD_ON_OTHER_KEY_PRESS_PER_KEY` 제거 및 `PERMISSIVE_HOLD` 적용.
- **Caps Word 더블탭 기능:** 좌측 외곽 Shift 키(`KC_LSFT`) 및 **양쪽 엄지 순수 Shift 키**(`KC_LSFT` / `KC_RSFT`)를 빠르게 **더블 탭**하여 Caps Word 상태를 즉시 활성화할 수 있습니다.
- **Layers:**
...
- **Key Position Changes for Optimization (인체공학 및 타이핑 효율성 극대화):**
  - **우측 가장자리 하단 키의 3단 변신 (윈도우 환경 최적화)**: 윈도우 환경의 한/영, 한자, 단축키 입력 호환성을 위해 우측 새끼손가락 외곽 하단 키 하나에 레이어별로 Windows 특화 키들을 통합 배치했습니다:
    - 기본 레이어 (`_DEFAULT`): **`KC_RALT` (한/영 전환)** - 단일 탭으로 언어 전환
    - 내비 레이어 (`_NAV`): **`KC_APP` (Context Menu / 우클릭 메뉴)** - 엄지 `SPC_NAV` 조합
    - 숫자 레이어 (`_NUM`): **`KC_RCTL` (한자 변환)** - 엄지 `TAB_NUM` 조합
